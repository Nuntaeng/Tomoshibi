...
public class LightMove : MonoBehaviour {
    ...
    void FixedUpdate () {
        ...
        //向きを参考に光の位置を調整
        posDest = new Vector3(status.direction.x * 0.5f, status.direction.y * 0.5f + 0.4f, 0.0f);
        this.transform.localPosition += (posDest - this.transform.localPosition) / 10.0f;
        //揺らめくみたいに移動させる
        this.transform.localPosition += new Vector3(Mathf.Cos(time) * 0.01f ,Mathf.Sin(time) * 0.01f, 0.0f);
        //光の明るさをセット
        float scale = status.lightPower + (Mathf.Sin(time) - 1) * (0.4f + detect) + status.minLightCircle;
        ...
        if (status.state != PlayerState.Dead)
        {
            ...
            {
                status.lightPower -= 0.001f;
                SetIntencity(status.lightPower);
            }
        }
        else
        {
            //死んだときにライトは消える
            //SetIntencity(-status.minLightCircle);
        }
        if (status.lightPower <= 0.0f || status.minLightCircle < 0.1f)
        {
